	...
	function customize_register( $wp_customize ){
		...
		if( method_exists($wp_customize, 'add_panel') ) {
			$wp_customize->add_panel( 'page_settings', array(
				'title' => __( 'Page Template Settings', 'influence' ),
				'description' => __( 'Change layouts for various pages on your site.', 'influence' ),
				'priority' => 11,
			) );
		}
		...
		$types = array();
		$templates = array(
			'home' => __( 'Blog Page', 'influence' ),
			'search' => __( 'Search Results', 'influence' ),
			'date' => __( 'Date Archives', 'influence' ),
			'author' => __( 'Author Archives', 'influence' ),
			'404' => __( 'Not Found', 'influence' ),
		);
		...
		foreach( $post_types as $post_type => $post_type_data ) {
			...
			$types[] = array(
				'group' => 'archive',
				'id' => $post_type,
				'title' => __( 'Type', 'influence' ) . ': ' . $post_type_data->label
			);
		}
		...
		foreach( $taxonomies as $tax_slug => $taxonomy ) {
			...
			$types[] = array(
				'group' => 'taxonomy',
				'id' => $tax_slug,
				'title' => __( 'Taxonomy', 'influence' ) . ': ' . $taxonomy->label
			);
		}
